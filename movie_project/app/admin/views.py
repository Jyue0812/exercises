...
def comment_list(page=None):
    if page is None:
        page = 1
    page_data = Comment.query.join(Movie).join(User). \
        filter(Movie.id == Comment.movie_id,
...
def moviecol_list(page=None):
    ...
    page_data = Moviecol.query.join(Movie).join(User). \
        filter(Movie.id == Moviecol.movie_id,
...
def role_add():
    ...
    if form.validate_on_submit():
        ...
        role = Role(
            name=data["name"],
            auths=",".join(map(lambda v: str(v), data["auth"]))
        )
        db.session.add(role)
        db.session.commit()
        flash("添加角色成功", "ok")
...
def role_edit(id):
    ...
    if request.method == "GET":
        auths = role.auths
        form.auth.data = list(map(lambda v: int(v), auths.split(",")))
    if form.validate_on_submit():
        data = form.data
        role.name = data["name"]
        role.auths = ",".join(map(lambda v: str(v), data["auth"]))
        db.session.add(role)
        db.session.commit()
        flash("修改角色成功！", "ok")
